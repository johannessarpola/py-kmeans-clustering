...
def vectorize(documents):
    ...
    logger.info(f"Total number of features in raw vector space: {len(vectorizer.feature_names_)}")
    return X, vectorizer
...
def do_lsa(X, n_components=100, n_iter=10, random_state=None):
    ...
    logger.info(f"Total features after LSA: {n_components}")
    return trunc_x, svd
